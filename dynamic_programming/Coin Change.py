class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        we will assign smallest dp[i + coin] + 1 to dp[i] (for every coin in coins and i + coin need to be smaller or equals to amount). dp[amount] will therefore be initialized to 0 (since we only need 0 when current amount is already equals to target). After iteration, we will return the answer, dp[0].
        """
        dp = [-1] * (amount + 1)
        dp[amount] = 0
